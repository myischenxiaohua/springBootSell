...
//        WxMpService wxMpService=new WxMpServiceImpl();
    ...
    @GetMapping("/pay")
    public String wxPay(@RequestParam("openid") String openid, Map map){
        PayRequest payRequest=new PayRequest();
        payRequest.setOpenid(openid);
        payRequest.setOrderId(KeyUtil.genUniqueKey());
        payRequest.setOrderAmount(1.0);
    ...
    @GetMapping("/ref")
    public String refund(Map map){
        RefundRequest refundRequest=new RefundRequest();
        refundRequest.setOrderId("1560849238611558913");
        refundRequest.setOrderAmount(1.0);
        refundRequest.setPayTypeEnum(BestPayTypeEnum.WXPAY_H5);
       RefundResponse refundResponse= bestPayService.refund(refundRequest);
       log.info("退款参数={}",JSON.toJSONString(refundResponse));
        map.put("refundResponse",refundRequest);
        return "pay/success";
